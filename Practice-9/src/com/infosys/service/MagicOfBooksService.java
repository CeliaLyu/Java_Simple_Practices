...
import com.infosys.exceptions.InvalidUserException;
import com.infosys.exceptions.InvalidBookException;
...
public class MagicOfBooksService {
        ...
        if (user != null && user.getPassword().equals(password)) {
            ...
            new Thread(() -> {
                try {
                    ...
                    // User session logic here...
                } catch (Exception e) {
                    System.out.println("Error during user session: " + e.getMessage());
                }
            }).start();
            return true;
        }
     ...
     */
    public Book getBook(int bookId) throws InvalidBookException {
     ...
     */
    public boolean deleteBook(int bookId) throws InvalidBookException {
     ...
     */
    public boolean markBook(int bookId, int markType) throws InvalidBookException {
        Book book = bookDao.getBook(bookId);
        if (book == null) {
            throw new InvalidBookException("Book not found.");
        }
        switch (markType) {
            case 1:
                currentUser.getNewBooks().add(book);
                break;
            case 2:
                currentUser.getFavouriteBooks().add(book);
                break;
            case 3:
                currentUser.getCompletedBooks().add(book);
                break;
            default:
                throw new InvalidBookException("Invalid mark type.");
        }
        return true;
    }
}
